...
2. **Asphalt fidelity:** #2 hex-tiling + #5 detail normals + #10 height-blend + #8 crown. The road surface itself stops reading procedural. **◐ IN PROGRESS** — #2 hex-tiling SHIPPED: `HEX_TILING_GLSL` in `src/materials/library.ts` (Mikkelsen offset-only, `textureGrad` explicit-gradient, contrast-sharpened barycentric blend) hex-tiles the realistic **asphalt albedo** to kill the ~6 m repeat. Correctly scoped to stochastic asphalt only — cobble/pavers/markings are excluded (hex-tiling fails on structured textures). Normal-map hex-tiling, #5 detail normals, #10 height-blend, #8 crown still to come.
