...
2. **Asphalt fidelity:** #2 hex-tiling + #5 detail normals + #10 height-blend + #8 crown. The road surface itself stops reading procedural. **✅ SHIPPED (3/4)** — all in `src/materials/library.ts`, applied only to realistic stochastic asphalt (cobble/pavers/markings excluded — hex-tiling & aggregate wear fail on structured textures; arcade stays clean):
   ...
   - **#8 crown/camber — DEFERRED** to a dedicated coordinated pass. A real geometric crown must thread a lateral term through `ribbonGeometry` (currently a 2-vertex-per-section flat strip), `surfaceElevSampler` (markings/decals/crosswalks project onto the centerline), AND `colliders.ts` (same centerline profile), then re-pass `flickerLint` + `colliderAudit`. Rushing it would break those load-bearing invariants, so it is intentionally its own change, not bundled here.
3. **Wear & markings:** #7 splat/wear mask + #12 decal atlas + #13 SDF markings. The "driven-in" look. **◐ IN PROGRESS** — #7 SHIPPED: lane-space wear in the asphalt shader (`src/materials/library.ts`) driven by an `aLane` (−1 kerb→+1 kerb) + `aWear` gate attribute set on carriageway ribbons in `roads.ts`. Polished wheel tracks + oily centerline (both lower roughness) + dusty kerb edge (higher roughness), with along-road modulation. Gated by `aWear` so junction patches / paths / cobble (no lane frame) are untouched — no misfire. #12 decal atlas + #13 SDF markings still to come.
4. **Integration & scale:** #3 terrain skirts + #15 corridor LOD + #9 InstancedMesh2 + #23 KTX2 bake. Makes it hold together and scale.
5. **Wow tier:** #14 wetness/puddles (+ #25 SSR/planar), #20 impostors, #21 TAA, #22 superelevation.
